#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
神经网络视频音频生成脚本
使用通用音频生成工具生成所有场景的音频文件
"""

import sys
from audio_generator import AudioGenerator, AudioGeneratorConfig, check_dependencies

# 场景配置
SCENES = {
    "scene1": {
        "name": "介绍",
        "text": "今天我们来学习神经网络算法。一句话核心：神经网络等于模拟人脑的计算网络，通过层层传递数据自动学习规律，输入、加工、输出是它的核心工作流。",
        "output_file": "scene1-audio.mp3",
        "caption_file": "scene1-captions.json"
    },
    "scene2": {
        "name": "定义",
        "text": "什么是神经网络？人工神经网络，简称ANN，是20世纪80年代以来人工智能领域兴起的研究热点。它从信息处理角度对人脑神经元网络进行抽象，建立某种简单模型，按不同的连接方式组成不同的网络。",
        "output_file": "scene2-audio.mp3",
        "caption_file": "scene2-captions.json"
    },
    "scene3": {
        "name": "三层功能",
        "text": "神经网络有三层功能。第一，输入层，接收数据，比如28乘28像素的手写数字图片。第二，隐藏层，层层提取特征，从线条到局部图案，再到完整数字。第三，输出层，给出预测结果，也就是概率最大的数字0到9。",
        "output_file": "scene3-audio.mp3",
        "caption_file": "scene3-captions.json"
    },
    "scene4": {
        "name": "怎么做",
        "text": "神经网络如何实现预测结果呢？第一步，神经元计算，像积木拼装一样，将所有数据、影响因子和权重都输入到网络中。第二步，激活函数，像质检开关，判断当前数据输出是否符合要求。第三步，损失函数，像误差雷达，计算预测值和实际值之间的误差。第四步，经过这三个步骤，不断迭代，直到误差最小，得到预测结果。",
        "output_file": "scene4-audio.mp3",
        "caption_file": "scene4-captions.json"
    },
    "scene5": {
        "name": "算法类型",
        "text": "神经网络有四种主要类型。全连接网络，每层神经元全部连接，典型应用是房价预测。卷积网络CNN，具有局部感知和权重共享的特点，用于人脸识别。循环网络RNN，能记忆之前的状态，应用于语音识别。还有Transformer，采用自注意力机制，就是ChatGPT使用的技术。",
        "output_file": "scene5-audio.mp3",
        "caption_file": "scene5-captions.json"
    },
    "scene6": {
        "name": "生活案例",
        "text": "我们用快递分拣中心来类比神经网络。收货区就是输入层，接收全国的包裹，也就是原始数据。分拣线就是隐藏层，首站按省份粗分，提取大特征；中转按城市细分，识别局部特征；末站按街道精分，确认细节。发货区就是输出层，送至具体地址，得到分类结果。",
        "output_file": "scene6-audio.mp3",
        "caption_file": "scene6-captions.json"
    },
    "scene7": {
        "name": "冷知识",
        "text": "最后分享一个冷知识。GPT-3的神经元数量是1750亿，大约是人类大脑神经元860亿的2倍。但是，人脑能耗仅20瓦，而训练GPT-3需要190万度电，相当于200个家庭的年用电量！",
        "output_file": "scene7-audio.mp3",
        "caption_file": "scene7-captions.json"
    }
}


def main():
    """生成所有场景的音频文件"""
    
    print("=" * 60)
    print("神经网络视频音频生成工具")
    print("=" * 60)
    print("🤖 使用Qwen3-TTS模型生成高质量语音解说")
    print("🎯 文本模式：直接从配置的文本生成音频和字幕")
    print("📝 字幕规则：按句子分割，一行一行显示")
    print("=" * 60)
    
    # 检查依赖
    if not check_dependencies():
        sys.exit(1)
    
    # 创建配置
    config = AudioGeneratorConfig(
        video_name="NeuralNetworkVideo",
        output_dir="public/NeuralNetworkVideo",
        model_path="./Qwen3-TTS-12Hz-1.7B-Base",
        reference_audio="./borfy.mp3",
        mode="text"  # 文本模式
    )
    
    # 创建生成器
    generator = AudioGenerator(config)
    
    # 生成音频
    success_count, skipped_count, failed_count = generator.generate_from_config(SCENES)
    
    # 输出结果
    print(f"\n📊 生成结果:")
    print(f"   总场景数: {len(SCENES)}")
    print(f"   成功处理: {success_count}/{len(SCENES)}")
    print(f"   使用已存在音频: {skipped_count}")
    print(f"   新生成音频: {success_count - skipped_count}")
    print(f"   失败: {failed_count}")
    
    if success_count == len(SCENES):
        print("\n🎉 所有音频和字幕文件生成完成！")
        print(f"📁 文件位置: {config.output_dir}/")
        print(f"   - 音频文件: scene*-audio.mp3")
        print(f"   - 字幕文件: scene*-captions.json")
        if skipped_count > 0:
            print(f"💡 跳过了 {skipped_count} 个已存在的音频文件")
        print("\n下一步:")
        print("1. 检查生成的音频和字幕文件")
        print("2. 运行 'npm run dev' 预览视频")
        print("3. 运行 'npm run render NeuralNetworkVideo' 渲染视频")
        sys.exit(0)
    else:
        print("\n⚠️  部分音频生成失败，请检查错误信息")
        sys.exit(1)


if __name__ == "__main__":
    main()