...
SCENE_CONFIG = {
    "scene1": {
        "name": "介绍",
        "caption_file": "scene1-intro-captions.json",
        "output_file": "scene1-intro.mp3"
    },
    "scene2": {
        "name": "定义和对比",
        "caption_file": "scene2-definition-captions.json",
        "output_file": "scene2-definition.mp3"
    },
    "scene3": {
        "name": "深度学习步骤",
        "caption_file": "scene3-process-captions.json",
        "output_file": "scene3-process.mp3"
    },
    "scene4": {
        "name": "经典案例",
        "caption_file": "scene4-case-study-captions.json",
        "output_file": "scene4-case-study.mp3"
    },
    "scene5": {
        "name": "体验深度模型",
        "caption_file": "scene5-experience-captions.json",
        "output_file": "scene5-experience.mp3"
    },
    "scene6": {
        "name": "里程碑",
        "caption_file": "scene6-milestones-captions.json",
        "output_file": "scene6-milestones.mp3"
    },
    "scene7": {
        "name": "总结与参考资料",
        "caption_file": "scene7-references-captions.json",
        "output_file": "scene7-references.mp3"
    },
    "scene8": {
        "name": "结尾收尾",
        "caption_file": "scene8-ending-captions.json",
        "output_file": "scene8-ending.mp3"
    }
}
...
def update_caption_timestamps(caption_file, audio_duration_ms):
    ...
    try:
        ...
        # 获取原始字幕的总时长
        original_duration_ms = captions[-1]['end']
        ...
        for caption in captions:
            caption['start'] = int(caption['start'] * scale_ratio)
            caption['end'] = int(caption['end'] * scale_ratio)
    ...
    for attempt in range(max_retries):
        ...
        try:
            wavs, sr = model.generate_voice_clone(
                ref_audio="../../borfy.mp3",
                ref_text="5分钟 AI，每天搞懂一个知识点！今天我们学习， 深度学习。",
                text=text,
                language="chinese",
...
def generate_all_scene_audios():
    ...
    print("⚡ 新增功能：自动检测已存在音频、读取音频时长、更新字幕时间戳")
    ...
    if success_count == len(SCENE_CONFIG):
        print("🎉 所有场景处理完成！")
        print("📁 音频文件位置: ./")
        if updated_caption_count == success_count:
            print("✅ 所有字幕时间戳已自动更新，与音频时长同步")
        else:
            print(f"⚠️  部分字幕时间戳更新失败 ({updated_caption_count}/{success_count})")
        if skipped_count > 0:
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("2. 运行 'npm run render -- DeepLearningVideo' 渲染最终视频")
        print("3. 音频文件位置: public/deeplearning/")
        print("💡 技术特点:")
        print("   • 使用Qwen3-TTS 1.7B模型")
        print("   • 支持语音风格定制，确保音色一致性")
        print("   • 自动读取字幕文件，无需手动配置文本")
        print("   • 根据文本内容自然生成音频时长")
        print("   • 自动更新字幕时间戳，与音频时长同步")
        print("   • 自动音量标准化优化")
        print("   • 高质量中文语音合成")
