...
1. **查找现有封面图**：自动搜索项目中已存在的封面图，用于参考风格
2. **风格参考检测**：如果找到现有封面图，分析其设计风格并生成风格描述
3. **生成提示词**：基于视频内容和现有风格，生成两种比例的封面图提示词
4. **调用 image-generator 技能**：使用生成的提示词通过 image-generator 技能生成封面图
